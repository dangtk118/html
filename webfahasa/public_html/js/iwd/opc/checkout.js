...
IWD.OPC.Checkout = {
		...
		init:function(){		
			...
			this.removePrice();
                        ...
                        var sm_container = $j("div#checkout-shipping-method-load");
                        var sm_w_vat = $j("div.fhs-shipping-methods");
		...
		updateShippingMethod: function(response){
                    var sm_container = $j(response.shipping).find("div#checkout-shipping-method-load");
                    var sm_w_vat = $j(response.shipping).find("div.fhs-shipping-methods");
                    this.handleUpdateShippingMethod(sm_container, sm_w_vat);
                },
                
                handleUpdateShippingMethod: function(sm_container, sm_w_vat){
                    var domexpress = jQuery(sm_w_vat.find(".price")[0]).html();
                    var domnormal = jQuery(sm_w_vat.find(".price")[1]).html();
                    if(typeof(domexpress) !== "undefined" && typeof(domnormal) !== "undefined"){
                        var expressShipVal = parseFloat(domexpress.replace("&nbsp;","").replace("đ", ""));
                        var normalShipVal = parseFloat(domnormal.replace("&nbsp;","").replace("đ", ""));
                        if(normalShipVal >= expressShipVal){
                            sm_w_vat.find("dt:eq(1)").remove();
                            sm_w_vat.find("dd:eq(1)").remove();
                            sm_container.find("dt:eq(1)").remove();
                            sm_container.find("dd:eq(1)").remove();
                        }
                    }                    
                    var cur_sm_w_vat = $j("div.fhs-shipping-methods");
                    if(cur_sm_w_vat.length === 0){
                        $j('div#checkout-shipping-method-load').html(sm_container.html());
                    }else{
                        $j('div.fhs-shipping-methods').html(sm_w_vat.html());
                    }
                },
		...
		prepareAddressResponse: function(response){
			...
			if (typeof(response.shipping) != "undefined"){
                            IWD.OPC.Checkout.updateShippingMethod(response);				
			}
			
			if (typeof(response.payments) != "undefined"){
				$j('#co-payment-form').empty().html(response.payments);
		...
		showLoader: function(){
			//$j('.opc-ajax-loader').show();
                        //$j('div.opc-col-right').block({message: null});
                        //$j('.fhs-opc-center-loader').show();                        
                        $j('div.opc-col-center').waitMe({
                            effect : 'bounce',
                            text : '',
		...
		hideLoader: function(){
			//$j('.opc-ajax-loader').hide();
                        //$j('div.opc-col-right').unblock();
			//$j('.opc-btn-checkout').removeClass('button-disabled');
                        //$j('.fhs-opc-center-loader').hide();
                        $j('div.opc-col-right').waitMe("hide");
                        $j('div.opc-col-center').waitMe("hide");
		},
		...
		pullPayments: function(){
			...
			IWD.OPC.Checkout.xhr = $j.post(IWD.OPC.Checkout.config.baseUrl + 'onepage/json/payments',function(response){
				...
				if (typeof(response.payments)!="undefined"){
					//$j('#co-payment-form').html(response.payments);
...
IWD.OPC.Billing = {
		...
		initChangeAddress: function(){
			...
			$j('#opc-address-form-billing input').change(function(){
		...
		validateForm: function(){
			setTimeout(function(){                            
                            ...
                            //if (valid){
                            var country_select = $j("select#billing\\:country_id").val();
                            if(country_select === "VN"){
                                var region_select = $j("select#billing\\:region_id").val();
                                var city_select = $j("select#billing\\:city_id").val();
                                if(region_select && city_select){
                                    IWD.OPC.Billing.save();
                                }
                            }else{
                                IWD.OPC.Billing.save();
...
IWD.OPC.Shipping = {
		...
		initChangeAddress: function(){
			...
			$j('#opc-address-form-shipping input').change(function(){
		...
		initChangeShippingMethod: function(){
			...
			$j('.opc-wrapper-opc #shipping-block-methods').on('change', 'input[type="radio"], input[type="checkbox"]', function(){
		...
		validateForm: function(){
			setTimeout(function(){
				...
				//if (valid){
                                var country_select = $j("select#shipping\\:country_id").val();
                                if(country_select === "VN"){
                                    var region_select = $j("select#shipping\\:region_id").val();
                                    var city_select = $j("select#shipping\\:city_id").val();
                                    if(region_select && city_select){
...
IWD.OPC.Login ={
		...
		init: function(){
			$j('.login-trigger').on('click', function(e){
				e.preventDefault();
				//$j('#modal-login').addClass('md-show');                                 
                                //setTimeout(function(){ $j('input.login-email').focus();}, 500);
                                $j('a[href*="customer/account/login"]').trigger('click');                                
			});
			...
			$j('#login-button-set .btn').not('.btn-facebook').click(function(){
				$j('#login-form').submit();
			});
