...
class Magentothem_Blog_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    const XML_TOP_CAT_ID = "blog/blog/catIds";
    const XML_TOP_POST_ID = "blog/blog/postIds";
    const XML_LEFT_POST_ID = "blog/blog/leftPostIds";
    ...
    public function getTopCatIds(){
        return $this->conf(self::XML_TOP_CAT_ID, null);
    }
    ...
    public function extensionEnabled($extensionName) {
        ...
        if (
                !isset($modules[$extensionName]) || $modules[$extensionName]->descend('active')->asArray() == 'false' || Mage::getStoreConfig('advanced/modules_disable_output/' . $extensionName)
        ) {
            return false;
        }
        return true;
    ...
    public function isMobileInstalled() {
        return $this->isModuleOutputEnabled('Magentothem_Mobile') && @class_exists('Magentothem_Mobile_Block_Catalog_Product_List_Toolbar')
        ;
    }

    public function limitString($str, $num) {
        $wraptext = wordwrap(trim(strip_tags($str)), $num, "---\n---", false);
        if (strpos($wraptext, "---\n---")) {
            $str = substr($wraptext, 0, strpos($wraptext, "---\n---")) . " ...";
        }
        return $str;
    }
    
    public function getTopViewed($pager, $limit) {
        if (is_numeric($pager) && is_numeric($limit)) {
            $connection = Mage::getSingleton('core/resource')->getConnection('core_read');
            $query = "select mb.*, bc.title as 'cat_title', bc.identifier as 'cat_identifier'
	    , bc.sort_order, bc.meta_keywords as 'cat_meta_keywords', bc.meta_description as 'cat_meta_description'
	    ...
	    order by mb.viewed DESC
	    limit ".$limit." OFFSET ".($pager*$limit).";";
            return $connection->fetchAll($query);
        }
    }
    
    public function getNewest($pager, $limit) {
        if (is_numeric($pager) && is_numeric($limit)) {
            $connection = Mage::getSingleton('core/resource')->getConnection('core_read');
            $query = "select mb.*, bc.title as 'cat_title', bc.identifier as 'cat_identifier'
	    , bc.sort_order, bc.meta_keywords as 'cat_meta_keywords', bc.meta_description as 'cat_meta_description'
	    ...
	    join fhs_magentothem_blog_cat bc on bc.cat_id = pc.cat_id
	    join fhs_magentothem_blog mb on mb.post_id = pc.post_id
	    order by mb.created_time DESC
	    limit ".$limit." OFFSET ".($pager*$limit).";";
            return $connection->fetchAll($query);
        }
    }
    
    public function getProductsByCat($catid, $sku){
	$collection = Mage::getResourceModel('catalog/product_collection')
		...
		->setPageSize($this->get_prod_count(9))
		->addCategoryFilter($category);
	$collection->getSelect()->where('e.sku not in (?)', $sku);
	return $collection;
    }
}
