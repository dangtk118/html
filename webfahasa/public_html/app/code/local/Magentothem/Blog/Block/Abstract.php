...
abstract class Magentothem_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    {
        return '<div><a class="magentothem-blog-read-more" href="' . $item->getAddress() . '">' . $this->__('Read More') . '</a></div>';
    }
...
//        $dateFormat = self::$_helper->getDateFormat();
        $createdTime = new DateTime($item->getCreatedTime());
        $updateTime = new DateTime($item->getUpdateTime());
        
        $item->setCreatedTime($createdTime->format('d/m/Y h:m:s'));
        $item->setUpdateTime($updateTime->format('d/m/Y h:m:s'));
