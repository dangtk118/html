...
class Magentothem_Ajaxcartsuper_AjaxcartController extends Mage_Checkout_CartController
{
    ...
    public function addAction()
    {
		...
		$type_product = !empty($params['type_product'])?$params['type_product']:0;
                if($type_product == 1) {
		...
		Mage::register('is_gat_customer_info_rest', true);
                $cart->save();
                ...
                if (!$this->_getSession()->getNoCartRedirect(true)) {
                    if (!$cart->getQuote()->getHasError()){
                       ...
                       // $this->_getSession()->addSuccess($message);
                        ...
                        $pimage = Mage::helper('catalog/image')->init($product, 'small_image')->resize(155);
                        ...
                        if(isset($messages['error'])){
                            $type = $messages['error']->getType();
                            $code = $messages['error']->getCode();                        
                            $ajaxData['type'] = $type;
                            $ajaxData['code'] = $code;
                            $errorMsg = "***ERROR WHEN ADD TO CART: session id: " . Mage::getSingleton("core/session")->getEncryptedSessionId() . 
                                ": Message: " . $code . " ***\n";
                            Mage::log($errorMsg);
                        }                        
                        Mage::log("*** MESSAGES: " . print_r($messages, true));
                        $smessages = Mage::getSingleton('checkout/session')->getMessages()->getItems();
                        $output = NULL;
                        if(isset($smessages) && !empty($smessages)){
                            foreach ($smessages as $smessage) {
                                $output .= $smessage->getText();
                            }
                           Mage::log("*** OUTPUT from checkout/session messages: " . print_r($output, true));
                        }
