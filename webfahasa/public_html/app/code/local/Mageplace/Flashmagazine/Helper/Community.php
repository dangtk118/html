...
class Mageplace_Flashmagazine_Helper_Community extends Mage_Core_Helper_Abstract
{
	...
	public function decodeHTML($string)
	{
		$string = strtr( $string, array_flip($this->_getHtmlTranslationTable( )));
                $string = preg_replace_callback( "/&#([0-9]+);/m", "chr", $string );
		return $string;
	}
	...
	public function convertEntity ($matches, $destroy = true)
	{
        ...
        public function getListPageImages($magazineListId) {
            $query = "select page_image,page_zoom_image from fhs_flashmagazine_page where page_magazine_id in (".implode(",",$magazineListId).") order by page_magazine_id, page_sort_order";
