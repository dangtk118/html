...
class Magestore_Onestepcheckout_Block_Onestepcheckout extends Mage_Checkout_Block_Onepage_Abstract {
    ...
    public function __construct() 
    {
        ...
        if($cartdata->getCountryId()){
			...
			if($cartdata->getWard()){
				$billing->setWard($cartdata->getWard());		
			}
			...
			if($cartdata->getWard()){
				$shipping->setWard($cartdata->getWard());		
			}							
		}else{
    ...
    */
	...
	if($type == 'billing' || $type == 'shipping' || empty($type)){
	    ...
	    $select = $this->getLayout()->createBlock('core/html_select')
                        ->setName($type.'_country_select')
                        ->setId($type.'_country_select')
                        ->setTitle(Mage::helper('onestepcheckout')->__('Country'))
                        ->setClass('fhs-selectbox')
                        ->setValue($countryId)
                        ->setOptions($this->getCountryOptions());
	}
    ...
    {
        $ward = $this->getAddress()->getWard();
        $primary = $this->getQuote()->getCustomer()->getPrimaryBillingAddress();         
        if(empty($ward)  && $this->_isLoggedIn() && $primary)	{
            return $this->getQuote()->getCustomer()->getPrimaryBillingAddress()->getWard();
        }
        return $ward;
    }
    ...
    {      		
        if ($shipping->getCountryId() == '' && isset($configData['country_id']) && $configData['country_id'] != '') $shipping->setCountryId($this->configData['country_id']);
        if ($shipping->getRegionId() == '' && isset($configData['region_id']) && $configData['region_id'] != '') $shipping->setRegionId($this->configData['region_id']);
        if ($shipping->getRegion() == '' && isset($configData['region_id']) && $configData['region_id'] != '') $shipping->setRegion($this->configData['region_id']);
        if ($shipping->getPostcode() == '' && isset($configData['postcode']) && $configData['postcode'] != '') $shipping->setPostcode($this->configData['postcode']);
        if ($shipping->getCity() == '' && isset($configData['city']) && $configData['city'] != '') $shipping->setCity($this->configData['city']);	
        if ($shipping->getWard() == '' && isset($configData['ward']) && $configData['ward'] != '') $shipping->setWard($this->configData['ward']);			
    }

    private function _setDefaultBillingAddress($billing,$configData) 
    {  
        if ($billing->getCountryId() == '' && isset($configData['country_id']) && $configData['country_id'] != '') $billing->setCountryId($this->configData['country_id']);
        if ($billing->getRegionId() == '' && isset($configData['region_id']) && $configData['region_id'] != '') $billing->setRegionId($this->configData['region_id']);
        if ($billing->getRegion() == '' && isset($configData['region_id']) && $configData['region_id'] != '') $billing->setRegion($this->configData['region_id']);
        if ($billing->getPostcode() == '' && isset($configData['postcode']) && $configData['postcode'] != '') $billing->setPostcode($this->configData['postcode']);
        if ($billing->getCity() == '' && isset($configData['city']) && $configData['city'] != '') $billing->setCity($this->configData['city']);		
        if ($billing->getWard() == '' && isset($configData['ward']) && $configData['ward'] != '') $billing->setWard($this->configData['ward']);		
    }
	/* End: Modified by Daniel - improve OSC load speed */
    ...
    public function getAddressesHtmlSelect($type)
    {
        if ($this->isCustomerLoggedIn()) {
            ...
            return "<div class='dropdownfhs'>" . $select->getHtml() . "</div>"
		    . "<script type='text/javascript'>var ".$type."_customer_address_list=".Mage::helper('core')->jsonEncode($options)."</script>";
        }
        return '';
    }
    ...
    public function getPaymentMethods()
    {
        ...
        $vnpayTutorial = Mage::getStoreConfig('payment/vnpay/vnp_Active_Tutorial');
        foreach ($payments as $paymentCode => $paymentModel) {
            if ($paymentModel->canUseCheckout()) {
                ...
                // add tutorial for vnpay : 
                if ($paymentCode == 'vnpay' && $vnpayTutorial) {
                    $vnpayBlockId = Mage::getStoreConfig('payment/vnpay/vnp_Block_Id');
                    $vnpayTutorialTitle = Mage::getStoreConfig('payment/vnpay/vnp_tutorial_title');
                    $methods[] = array(
                        'label' => $paymentTitle,
                        'value' => $paymentCode,
                        'tutorialID' => $vnpayBlockId,
                        'tutorialTitle' => $vnpayTutorialTitle,
                    );
                } else {
                    $methods[] = array(
                        'label' => $paymentTitle,
