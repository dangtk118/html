...
class Mana_Seo_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getActiveSchema($storeId, $flat = true) {
	    ...
	    $cache_key = 'seo_schema_'.$storeId.'_'($flat?'1':'0');
		...
		$data = array();
		foreach($this->_activeSchemas as $key=>$item){
		    $data[$key] = $core_helper->jsonEncode($item);
		}
		$cache_data = array(
		    'data' => $data,
		    'model' => 'Mana_Seo_Model_Schema',
		    'cached' => true
		);
		$cache_helper->setData($cache_key, $cache_data);
	    }else{
		if(!empty($cache_data['model'])){
		    $schemas = $cache_helper->getCacheResult($cache_data);
		    if(!empty($schemas)){
			foreach($schemas as $key=>$item){
			    $this->_activeSchemas[$key] = $item;
			}
		    }
		}else{
			...
			foreach($cache_data['data'] as $key=>$item){
			    if($flat){
				$this->_activeSchemas[$key] = Mage::getModel('Mana_Seo_Model_Schema')->setData($core_helper->jsonDecode($item));
			    }else{
				$this->_activeSchemas[$key] = Mage::getModel('Mana_Seo_Model_Schema')->setData($core_helper->jsonDecode($item));
			    }
			    $this->_activeSchemas[$key]->setIdFieldName("id");
			}
		    }
		}
	    }
