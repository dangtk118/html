...
class Mana_Seo_Helper_UrlParser extends Mage_Core_Helper_Abstract  {
                    ...
                    if ($this->_getParameterUrlKey($token)) {
                        ...
                        switch ($token->getParameterUrl()->getType()) {
                            ...
                            case Mana_Seo_Model_ParsedUrl::PARAMETER_SPECIAL:
                                if ($this->_parseSpecialValues($token)) {
                                    return true;
                                }
                                break;
                            default:
     ...
     * @return bool
     */
    protected function _parseSpecialValues($token) {
        $cNotFound = Mana_Seo_Model_ParsedUrl::CORRECT_NOT_FOUND_ATTRIBUTE_FILTER_URL_KEY;
        $cExpectedParameterName = Mana_Seo_Model_ParsedUrl::CORRECT_EXPECTED_PARAMETER_NAME_FOR_ATTRIBUTE_FILTER_URL_KEY;
        $cRedundantParameterName = Mana_Seo_Model_ParsedUrl::CORRECT_REDUNDANT_PARAMETER_NAME_FOR_ATTRIBUTE_FILTER_URL_KEY;
        ...
        if (($text = $token->getTextToBeParsed()) && ($tokens = $this->_scanUntilSeparator($token, $this->_schema->getMultipleValueSeparator()))) {
            // get all valid attribute value URL keys
            if ($tokens = $this->_getSpecialValueUrlKeys($tokens)) {
                foreach ($tokens as $token) {
                    if ($token->getSpecialValueUrl()->getFinalIncludeFilterName()) {
                        $token->setPendingCorrection($cExpectedParameterName);
                        if (!$token->getParameterUrl()) {
                            if (!$this->_correct($token, $cExpectedParameterName, __LINE__, $text)) {
                                return false;
	...
	//@note: 
	//---------------
	$cache_key = 'seo_schema_'.Mana_Seo_Model_Schema::STATUS_ACTIVE.'_'.Mana_Seo_Model_Schema::STATUS_OBSOLETE;
	...
	if(empty($cache_data)){
	    ...
	    $cache_data = array(
		'data' => array('activeSchemas' => $activeSchemas,'obsoleteSchemas' => $obsoleteSchemas),
		'cached' => true
	    );
	    $cache_helper->setData($cache_key, $cache_data);
	}else{
	    if(empty($cache_data['activeSchemas'])){
		$data = $cache_helper->getCacheResult($cache_data);
		if(!empty($data)){
		    $activeSchemas = $data['activeSchemas'];
		    $obsoleteSchemas = $data['obsoleteSchemas'];
		}
	    }else{
		//clear this code late
		$activeSchemas = $cache_data['activeSchemas'];
		$obsoleteSchemas = $cache_data['obsoleteSchemas'];
	    }
	}
    }
	...
	foreach($suffixes as $key=>$item){
	    $cache_key .= "_".$key."_".$item;
	}
	$cache_helper = Mage::helper('fahasa_catalog/cache');
	$cache_data = $cache_helper->getData($cache_key);
	if(empty($cache_data)){
	    ...
	    $suffixes = $cache_helper->getCacheResult($cache_data);
	}
    ...
    {
	if(!empty($this->_path)){
	    if(($this->isStartsWith($this->_path, 'tabslider'))
		||($this->isStartsWith($this->_path, 'event'))
		||($this->isStartsWith($this->_path, 'onestepcheckout'))
		||($this->isStartsWith($this->_path, 'api'))
		||($this->isStartsWith($this->_path, 'coupon'))
		||(($this->isStartsWith($this->_path, 'catalog')) && !($this->isStartsWith($this->_path, 'catalogsearch')))
		||($this->isStartsWith($this->_path, 'wishlist'))
		||($this->isStartsWith($this->_path, 'customer'))
		||($this->isStartsWith($this->_path, 'newsletter'))
		||($this->isStartsWith($this->_path, 'tryout'))
		||($this->isStartsWith($this->_path, 'review'))
		||($this->isStartsWith($this->_path, 'blog'))
		||($this->isStartsWith($this->_path, 'admins'))
		||($this->isStartsWith($this->_path, 'turpentine'))
		||(($this->isStartsWith($this->_path, 'daily-deals')) && (!$this->isStartsWith($this->_path, 'daily-deals/page')))
		||($this->isStartsWith($this->_path, 'flashsale'))
		||($this->isStartsWith($this->_path, 'fpoint-store'))
		||($this->isStartsWith($this->_path, 'he-thong-trung-tam-nha-sach'))
		||($this->isStartsWith($this->_path, 'sales'))
		||($this->isStartsWith($this->_path, 'ajaxlogin'))
		||($this->isStartsWith($this->_path, 'daily-suggestion'))
		||($this->isStartsWith($this->_path, 'dieu-khoan-su-dung'))
		||($this->isStartsWith($this->_path, 'chinh-sach-bao-mat'))
		||($this->isStartsWith($this->_path, 'gioi-thieu-fahasa'))
		||($this->isStartsWith($this->_path, 'doi-tra-hang'))
		||($this->isStartsWith($this->_path, 'chinh-sach-khach-si'))
		||($this->isStartsWith($this->_path, 'phuong-thuc-van-chuyen'))
		||($this->isStartsWith($this->_path, 'huong-dan-thanh-toan'))
		||($this->isEqual($this->_path, 'foreign-books-coming-soon'))
		||($this->isStartsWith($this->_path, 'installtemplate'))
		||($this->isStartsWith($this->_path, 'vietnamshipping'))
		||($this->isStartsWith($this->_path, 'revslideradmin'))
		||($this->isStartsWith($this->_path, 'codfee/admin'))
		||($this->isStartsWith($this->_path, 'fieldsmanageradmin'))
		||($this->isStartsWith($this->_path, 'banner7/admin'))
		||($this->isStartsWith($this->_path, 'brandslider'))
		||($this->isStartsWith($this->_path, 'searchindex/adminhtml'))
		||($this->isStartsWith($this->_path, 'searchlandingpage/adminhtml'))
		||($this->isStartsWith($this->_path, 'searchsphinx/adminhtml'))
		||($this->isStartsWith($this->_path, 'phanloaivct'))
		||($this->isStartsWith($this->_path, 'ajax'))
		||($this->isStartsWith($this->_path, 'media'))
		||($this->isEqual($this->_path, 'usborne-publishing'))
		||($this->isEqual($this->_path, 'sterling-publishing'))
		||($this->isEqual($this->_path, 'penguin-publishing'))
		||($this->isEqual($this->_path, 'pearson-elt'))
		||($this->isEqual($this->_path, 'parragon-publishing'))
		||($this->isEqual($this->_path, 'oxford-elt'))
		||($this->isEqual($this->_path, 'macmillan-elt'))
		||($this->isEqual($this->_path, 'mcgraw-hill-publishing'))
		||($this->isEqual($this->_path, 'hachette-publishing'))
		||($this->isEqual($this->_path, 'harper-collins'))
		||($this->isEqual($this->_path, 'cengage-elt'))
		||($this->isEqual($this->_path, 'cambridge-elt'))
		||($this->isEqual($this->_path, 'redeem'))
		||($this->isEqual($this->_path, 'ykienkhachhang'))
		||($this->isEqual($this->_path, 'bach-viet'))
		||($this->isEqual($this->_path, 'van-lang-books'))
		||($this->isEqual($this->_path, 'dong-a'))
		||($this->isEqual($this->_path, 'mc-books'))
		||($this->isEqual($this->_path, 'do-choi-duka'))
		||($this->isEqual($this->_path, 'do-choi-smart-marketing'))
		||($this->isEqual($this->_path, 'artline'))
		||($this->isEqual($this->_path, 'minh-long'))
		||($this->isEqual($this->_path, 'happy-time'))
		    ){
		//Mage::log("---------out----------key:".$this->_path, null, "seo.log");
		return;
	    }
	    else{
		//Mage::log("---------in----------key:".$this->_path, null, "seo.log");
	    }
	}
        /* @var $dbHelper Mana_Db_Helper_Data */
        $dbHelper = Mage::helper('mana_db');
	...
	//@note: cache
	//---------------
	$cache_key = 'seo_url_'.$this->_schema->getId().'_'.$this->_schema->getData('accent_insensitive').'_'.Mana_Seo_Resource_Url_Collection::TYPE_PAGE;
	if ($flatTokens !== false) {
	    foreach(array_keys($flatTokens) as $item) {
		$cache_key .= "_".$item;
	    }
	}
	...
	$urls = array();
	if(empty($cache_data)){
	    $urls_data = $this->_getUrls($flatTokens, Mana_Seo_Resource_Url_Collection::TYPE_PAGE);
	    if($urls_data){
		$urls_cache = array();
		...
		$cache_data = array(
		    'data' => $urls_cache,
		    'model' => 'Mana_Seo_Model_Url',
		    'cached' => true
		);
		$cache_helper->setData($cache_key, $cache_data);
	    }
	}else{
	    $urls = $cache_helper->getCacheResult($cache_data);
	}
	...
	//@note: cache
	//---------------
	$cache_key = 'seo_url_'.$this->_schema->getId().'_'.$this->_schema->getData('accent_insensitive').'_'.Mana_Seo_Resource_Url_Collection::TYPE_ATTRIBUTE_VALUE;
	$token = current($tokens);
	if (($attributeId = $token->getAttributeId()) !== false) {
	    $cache_key .= '_'.$attributeId;
	}
	if ($tokens !== false) {
	    foreach(array_keys($tokens) as $item) {
		$cache_key .= "_".$item;
	    }
	}
	$core_helper = Mage::helper('core');
	...
	if(empty($cache_data)){
	    ...
	    $urls_data = $this->_getUrls($tokens, Mana_Seo_Resource_Url_Collection::TYPE_ATTRIBUTE_VALUE);
	    if(!empty($urls_data)){
		/* @var $token Mana_Seo_Model_ParsedUrl */
		if ($attributeId){
		    $urls_data->addFieldToFilter('attribute_id', $attributeId);
		}
		$urls_cache = array();
		foreach($urls_data as $item){
		    $urls[] = $item;
     ...
     * @return Mana_Seo_Model_ParsedUrl[] | bool
     */
    protected function _getSpecialValueUrlKeys($tokens) {
        if (!$tokens) {
            return false;
        }
        $result = array();
	...
	//@note: cache
	//---------------
	$cache_key = 'seo_url_'.$this->_schema->getId().'_'.$this->_schema->getData('accent_insensitive').'_'.Mana_Seo_Resource_Url_Collection::TYPE_ATTRIBUTE_VALUE.'_'.Mana_Seo_Model_ParsedUrl::PARAMETER_SPECIAL;
	if ($tokens !== false) {
	    foreach(array_keys($tokens) as $item) {
		$cache_key .= "_".$item;
	    }
	...
	if(empty($cache_data)) {
	    ...
	    if($urls_data){
		/* @var $token Mana_Seo_Model_ParsedUrl */
		$urls_data->addFieldToFilter('type', Mana_Seo_Model_ParsedUrl::PARAMETER_SPECIAL);
		$urls_cache = array();
		foreach($urls_data as $item){
		    $urls[] = $item;
		    $urls_cache[] = $core_helper->jsonEncode($item);
	...
	if(!empty($urls)){
		...
		if (isset($result[$finalUrlKey])) {
		    /* @var $conflictingToken Mana_Seo_Model_ParsedUrl */
		    $conflictingToken = $result[$finalUrlKey];
		    if ($url->getStatus() == Mana_Seo_Model_Url::STATUS_ACTIVE && $conflictingToken->getSpecialValueUrl()->getFinalIncludeFilterName()) {
			unset($result[$finalUrlKey]);
		    }
		}
		if (!isset($result[$finalUrlKey])) {
		    $token = $tokens[$finalUrlKey];
		    $token->setSpecialValueUrl($url);
		    $this->_activate($token, $url->getStatus() == Mana_Seo_Model_Url::STATUS_ACTIVE);
		    $result[$finalUrlKey] = $token;
		}
		elseif ($url->getStatus() == Mana_Seo_Model_Url::STATUS_ACTIVE && !$url->getFinalIncludeFilterName()) {
	...
	if (!isset($this->_parameterUrls[$this->_schema->getId()])) {
	    ...
	    //@note: cache
	    //---------------
	    $cache_key = 'seo_url_'.$this->_schema->getId().'_'.$this->_schema->getData('accent_insensitive').'_'.Mana_Seo_Resource_Url_Collection::TYPE_PARAMETER.'_0';
	    $core_helper = Mage::helper('core');
	    $cache_helper = Mage::helper('fahasa_catalog/cache');
	    $cache_data = $cache_helper->getData($cache_key);
	    $urls = array();
	    if(empty($cache_data)) {
		$urls_data = $this->_getUrls(false, Mana_Seo_Resource_Url_Collection::TYPE_PARAMETER);
		if(!empty($urls_data)){
		    $urls_cache = array();
		    foreach($urls_data as $item){
			$urls[] = $item;
		...
		$urls_result = array();
		foreach ($urls as $url) {
		    if (!isset($urls_result[$url->getFinalUrlKey()])) {
			$urls_result[$url->getFinalUrlKey()] = $url;
		    }
		    else {
			$this->_conflict($url->getFinalUrlKey(), self::CONFLICT_PARAMETER);
		    }
		}
		$this->_parameterUrls[$this->_schema->getId()] = $urls_result;
	    }
	}
	...
	//@note: cache
	//---------------
	$cache_key = 'seo_url_'.$this->_schema->getId().'_'.$this->_schema->getData('accent_insensitive').'_'.Mana_Seo_Resource_Url_Collection::TYPE_CATEGORY_VALUE;
	$token = current($tokens);
	$cache_key .= '_'.$token->getCategoryPath();
	...
	if(empty($cache_data)) {
	    /* @var $urlCollection Mana_Seo_Resource_Url_Collection */
	    $urls_data = $this->_getUrls($tokens, Mana_Seo_Resource_Url_Collection::TYPE_CATEGORY_VALUE);
	    if(!empty($urls_data)){
		/* @var $token Mana_Seo_Model_ParsedUrl */
		$urls_data->addParentCategoryFilter($token->getCategoryPath());
		$urls_cache = array();
		foreach($urls_data as $item){
		    $urls[] = $item;
		    $urls_cache[] = $core_helper->jsonEncode($item);
     ...
     * @return bool
     */
    protected function _setSpecialFilter($token) {
        if ($this->coreHelper()->isSpecialPagesInstalled()) {
            $token->addQueryParameter(Mage::helper('mana_page/special')->getRequestVar(), $token->getSpecialValueUrl()->getUrlKey());
        }
    ...
    function isEqual($haystack, $needle) {
