...
class Mana_Seo_Model_Observer {
    ...
    protected function _getFilters() {
		...
		//@note: cache
		//---------------
		$cache_key = 'filters_'.$this->coreHelper()->isManadevSeoLayeredNavigationInstalled().'_1';
		$core_helper = Mage::helper('core');
		$cache_helper = Mage::helper('fahasa_catalog/cache');
		$cache_data = $cache_helper->getData($cache_key);
		$filters = array();
		if (empty($cache_data)) {
		    $filters_data = $this->filterHelper()->getFilterOptionsCollection(true);
		    foreach ($filters_data as $item) {
			$filters[] = $item;
			$filters_cache[] = $core_helper->jsonEncode($item);
		    }
		    $cache_data = array(
			'data' => $filters_cache,
			'model' => 'Mana_Filters_Model_Filter2_Store',
			'cached' => true
		    );
		    $cache_helper->setData($cache_key, $cache_data);
		} else {
		    $filters = $cache_helper->getCacheResult($cache_data);
		}
		...
		$filters_data = Mage::getResourceModel('catalog/product_attribute_collection');
		    $filters_data->addIsFilterableFilter();
		...
		//@note: cache
		//---------------
		$cache_key = 'filters_'.$this->coreHelper()->isManadevSeoLayeredNavigationInstalled();
		$core_helper = Mage::helper('core');
		$cache_helper = Mage::helper('fahasa_catalog/cache');
		$cache_data = $cache_helper->getData($cache_key);
		$filters = array();
