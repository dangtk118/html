...
//        self::_resetFreeItems(Mage::helper("rediscart/cart")->getStaticQuote());
    }
    ...
    protected static function _handleGift(Mage_Sales_Model_Quote $quote, 
        ...
        if ($qty) {
            ...
            //check if applied rule is equal with rules in carts, because carts has many rule which add freegift and customer can choose a wrong rule_id
            if ($freeItem && ($rule->getAutoAddGift() || $quote->getAppliedRulesGift() == $rule->getId())) {
    ...
    protected static function _getFreeQuoteItem(Mage_Sales_Model_Quote $quote, $sku, $storeId, $qty)
    {
        ...
        //Trung: Check if we have sufficient gift to fulfill        
