...
class Bubble_Queryfier_Block_Page_Html_Head extends Mage_Page_Block_Html_Head
{
    ...
    public function getNewTitle()
    {
        ...
        if ($category){
            $newTitle = $category->getDescription();
            if ($newTitle == null) {
                 $this->_data['title'] = htmlspecialchars(html_entity_decode(trim($this->_data['title']), ENT_QUOTES, 'UTF-8'));
            } else {
                $this->_data['title'] = $category->getDescription();
            }
        } else {
	...
	$url = $this->getParentBlock()->getRequest()->getPathInfo();
	if($this->getParentBlock()->getRequest()->getAliases()){
	   $url = $this->getParentBlock()->getRequest()->getAliases()['rewrite_request_path'];
	}
	
	return Mage::helper('fahasa_catalog/cache')->getPageKeyWord($url);
    }
}
