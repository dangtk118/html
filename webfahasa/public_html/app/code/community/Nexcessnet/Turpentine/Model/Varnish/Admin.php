...
class Nexcessnet_Turpentine_Model_Varnish_Admin {
    ...
    public function flushAll() {
        try {
            $restSocket = $this->_getRestApiVarnishSocket();
            $restSocket->ban('req.http.host', '~', '.*');
        }
        catch (Exception $e) {
        }
    ...
    /* Trung: Return the socket to Fahasa Magento Rest Api Varnish */
    protected function _getRestApiVarnishSocket() {
        $key = str_replace( '\n', PHP_EOL,
        ...
        $server_sockets = Mage::helper('turpentine/varnish')->getSockets();
        if (count($server_sockets) > 0) {
            $server_socket = $server_sockets[0];
            return Mage::helper('turpentine/varnish')->getSocket($server_socket->getHost(), '6083', $key, $version);
        } else {
            Mage::getSingleton('adminhtml/session')->addError('There is no server list in Varnish Config');
            return Mage::helper('turpentine/varnish')->getSocket('127.0.0.1', '6083', $key, $version);
        }
    }
}
