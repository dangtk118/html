...
//	}
	...
	if(Mage::getSingleton('customer/session')->isLoggedIn()){
        ...
        if ($ruleId && $rule->getUsesPerCustomer()) {
         ...
         //vannguyen: this behind code is commented for optimization salesrule. Because there are too many query for each rules
        //we don't consider customer_group_id + website_id but we consider salesrule_coupon.
        //the function handle for checking customer_group_id and website_id is in init funtion at Mage_SalesRule_Model_Validator class 
        
        //handle for customer_group_id + website_id + salesrule_coupon
//        $rule->afterLoad(); 
//        
        //handle for salesrule_coupon
        $rule->_afterLoad();
        ...
        if(isset($coupon)){
            Mage::log("*** Coupon Validator. Coupon passed validated: '" . $couponCode . "'. Usuage Limit: " . $coupon->getUsageLimit() 
                . ", Times Used: " . $coupon->getTimesUsed() . ", Usage Per Customer: " . $coupon->getUsagePerCustomer() 
                . ", Expiration: " . $coupon->getExpirationDate(), null, "magento.log");
        }        
        /**
     ...
     * @return  Mage_SalesRule_Model_Validator
     */
    public function process(Mage_Sales_Model_Quote_Item_Abstract $item, & $max_discount_items = [])
    {
	    ...
	    //check max discount fee
	    if($discountAmount > 0){
		if($rule->getMaxDiscountFee()){
		    $old_DiscountAmount = 0;
		    $old_BaseDiscountAmount = 0;
		    if($max_discount_item = $max_discount_items[$rule->getRuleId()]){
			$old_DiscountAmount = $max_discount_item['discountAmount'];
			$old_BaseDiscountAmount = $max_discount_item['baseDiscountAmount'];
			$max_discount_item['discountAmount'] = (float)$max_discount_item['discountAmount'] + (float)$discountAmount;
			$max_discount_item['baseDiscountAmount'] = (float)$max_discount_item['baseDiscountAmount'] + (float)$baseDiscountAmount;
		    }else{
			$max_discount_item = [];
			$max_discount_item['max_discount'] = $rule->getMaxDiscountFee();
			$max_discount_item['discountAmount'] = (float)$discountAmount;
			$max_discount_item['baseDiscountAmount'] = (float)$baseDiscountAmount;
		    }
		    $max_discount_items[$rule->getRuleId()] = $max_discount_item;
		    
		    if($old_DiscountAmount > $max_discount_item['max_discount']){
			$discountAmount = 0;
			$baseDiscountAmount = 0;
		    }else{
			if(($discountAmount + $old_DiscountAmount) >= $max_discount_item['max_discount']){
			    $max_discount_result = $max_discount_item['max_discount'] - $old_DiscountAmount;
			    $discountAmount = $max_discount_result;
			    $baseDiscountAmount = $max_discount_result;
			}
		    }
		}
	    }
