...
class Mage_Core_Model_Translate
{
	...
	//@note: cache
	//---------------
	$cache_key = 'translation_'.$this->getLocale();
	$cache_helper = Mage::helper('fahasa_catalog/cache');
	$cache_data = $cache_helper->getData($cache_key);
	if (empty($cache_data)) {
	    $result = $this->getResource()->getTranslationArray(null, $this->getLocale());
	    $cache_data = array(
		'data' => $result,
		'cached' => true
	    );
        ...
        $this->_addData($result, $this->getConfig(self::CONFIG_KEY_STORE), $forceReload);
        return $this;
    }
