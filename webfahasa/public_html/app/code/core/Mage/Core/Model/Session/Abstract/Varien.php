...
class Mage_Core_Model_Session_Abstract_Varien extends Varien_Object
{
    ...
    /**
     * Tell if this session exists previously
     * @var boolean
     */
    protected static $_isExistedSession = null;
    
    public function isExistedSession() {
        return self::$_isExistedSession;
    }
...
//            Mage::log("getTotals session start  --- " . $time, null, "time.log");
        if (self::$_isExistedSession == null) {
            if ($_SESSION == null || empty($_SESSION)) {
                self::$_isExistedSession = FALSE;
            } else {
                self::$_isExistedSession = TRUE;
            }
        }

        if ($_SERVER['REQUEST_METHOD'] == "GET" &&
                (strpos($_SERVER['REQUEST_URI'], "/turpentine/esi/get") == 0 || $_SERVER['HTTP_X_TURPENTINE_SECRET_HANDSHAKE'] ==  "1")){
            
        }
        else if (Mage::app()->getFrontController()->getRequest()->isSecure() && empty($cookieParams['secure'])) {
