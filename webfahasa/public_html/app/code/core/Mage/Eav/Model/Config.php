...
class Mage_Eav_Model_Config
{
    ...
    public function getEntityType($code)
    {
	    ...
	    //@note: cache
	    //---------------
	    $cache_key = 'eav/entity_type__'.$code;
		...
		$cache_data = array(
		    'data' =>  $core_helper->jsonEncode($entityType),
		    'model' => 'Mage_Eav_Model_Entity_Type',
		    'cached' => true
		);
		$cache_helper->setData($cache_key, $cache_data);
	    }else{
		$entityType = $cache_helper->getCacheResult($cache_data);
	    }
