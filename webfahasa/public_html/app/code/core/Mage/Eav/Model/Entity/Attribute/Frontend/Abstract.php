    ...
    implements Mage_Eav_Model_Entity_Attribute_Frontend_Interface
{
     ...
     * @return mixed
     */
    public function getValue(Varien_Object $object, $is_array = false)
    {
        $value = $object->getData($this->getAttribute()->getAttributeCode());
        if (in_array($this->getConfigField('input'), array('select','boolean'))) {
            $valueOption = $this->getOption($value, $is_array);
            if (!$valueOption) {
            ...
            $value = $this->getOption($value, $is_array);
	    if (!empty($value)) {
		if($is_array){
		    if (is_array($value['values'])) {
			$value['values'] = implode(', ', $value['values']);
		    }
		}else{
		    if (is_array($value)) {
			$value = implode(', ', $value);
     ...
     */
    public function getOption($optionId, $is_array = false)
    {
        $source = $this->getAttribute()->getSource();
        if ($source) {
	    if($is_array){
		return $source->getOptionArray($optionId, $is_array);
	    }else{
		return $source->getOptionText($optionId, $is_array);
	    }
        }
        return false;
    }
