...
class Mage_Eav_Model_Resource_Entity_Attribute extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    protected function _loadTypeAttributes($entityTypeId)
    {
	    ...
	    //@note: cache
	    //---------------
//	    $cache_key = 'eav_entity_type_loadTypeAttributes'.$entityTypeId;
...
//	    } else {
//		$result = $cache_helper->getCacheResult($cache_data);
//	    }
	...
	//@note: cache
	//---------------
//	$cache_key = 'eav_entity_type_attribute_id_'.$entityType.'_'.$code;
//	$cache_helper = Mage::helper('fahasa_catalog/cache');
//	$cache_data = $cache_helper->getData($cache_key);
//	if(empty($cache_data)) {
	    $adapter = $this->_getReadAdapter();
...
//	}
    ...
    protected function _afterLoad(Mage_Core_Model_Abstract $object)
    {
	    ...
	    //@note: cache
	    //---------------
	    $cache_key = 'eav_entity_type_afterLoad_entity_type_id'.$object->getEntityTypeId();
	    $cache_helper = Mage::helper('fahasa_catalog/cache');
	    $cache_data = $cache_helper->getData($cache_key);
	    if(empty($cache_data)) {
		$additionalTable = $this->getAdditionalAttributeTable($object->getEntityTypeId());
		...
		$additionalTable = $cache_helper->getCacheResult($cache_data);
	    }
        }
	    ...
	    //@note: cache
	    //---------------
	    $cache_key = 'eav_entity_type_afterLoad_attribute_id'.$object->getId();
	    $cache_helper = Mage::helper('fahasa_catalog/cache');
	    $cache_data = $cache_helper->getData($cache_key);
	    if(empty($cache_data)) {
		$adapter = $this->_getReadAdapter();
