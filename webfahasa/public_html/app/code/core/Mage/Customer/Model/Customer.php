...
class Mage_Customer_Model_Customer extends Mage_Core_Model_Abstract
{
    ...
    const XML_PATH_OTP_TEMPLATE = 'customer/create_account/customer_email_otp_template';
    ...
    {
        $this->_getResource()->loadByTelephone($this, $customerTelephone);
        return $this;
    }
     ...
     * @return  Mage_Customer_Model_Customer
     */
    public function loadByFacebookId($facebook_id)
    {
	$readConnection = Mage::getSingleton('core/resource')->getConnection('core_read');
	$query = "select e.entity_id
		from fhs_facebook_user f 
		join fhs_customer_entity e on e.entity_id = f.customer_id
		where f.facebook_id = '".$facebook_id."';";
	$rs = $readConnection->fetchAll($query);
	if(!empty($rs[0])){
	    return $this->load($rs[0]);
	}
        return $this;
    }
    ...
    public function validatePassword($password)
    {
        ...
        /* Trung: for mobile, we accept hashed password as input.*/
        $hashArr = explode(':', $hash);
        $hashedPassword = $hashArr[0];
	...
	$notverify = "notverify";
	$domain = "@fahasa.com";
	if((substr($email, 0, strlen($notverify)) === $notverify) 
    ...
    public function sendOTPEmail($otp = '', $storeId = '0')
    {
        if (!$storeId) {
            $storeId = $this->_getWebsiteStoreId($this->getSendemailStoreId());
        }

        $this->_sendEmailTemplate(self::XML_PATH_OTP_TEMPLATE, self::XML_PATH_REGISTER_EMAIL_IDENTITY,
            array('customer' => $this, 'otp' => $otp), $storeId);

        return $this;
    }
    
     /**
     * Load customer by Apple Id
     *
     * @param   string $apple_id
     * @return  Mage_Customer_Model_Customer
     */
    public function loadByAppleId($apple_id)
    {
	$readConnection = Mage::getSingleton('core/resource')->getConnection('core_read');
	$query = "select e.entity_id
		from fhs_apple_user a 
		join fhs_customer_entity e on e.entity_id = a.customer_id
		where a.apple_id = '".$apple_id."';";
	$rs = $readConnection->fetchAll($query);
	if(!empty($rs[0])){
	    return $this->load($rs[0]);
	}
