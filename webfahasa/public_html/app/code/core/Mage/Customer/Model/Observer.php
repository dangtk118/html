...
class Mage_Customer_Model_Observer
{
    ...
    public function actionUpgradeCustomerPassword($observer)
    {
        ...
        Mage::log("------------------------------", null, "debug_password.log");
        Mage::log("Begin: Action upgrade customer password: email=" . $model->getEmail() . ", customer_id=" . $model->getId()
                . ", password=" . $password . ", old_hash_password=" . $model->getPasswordHash(), null, "debug_password.log");
        ...
        $hashData = $model->getPasswordHash();
        $hashArr = explode(':', $hashData);
        $hashPassword = $hashArr[0];
        
        if (Mage_Core_Model_Encryption::HASH_VERSION_SHA256 !== $currentVersionHash && $hashPassword != $password ){
            $model->changePassword($password, false);

            Mage::log("INSIDE: Change password: email=" . $model->getEmail() . ", customer_id=" . $model->getId()
                    . ", password=" . $password . ", new_hash_password=" . $model->getPasswordHash(), null, "debug_password.log");
        }
        Mage::log("End: Action upgrade customer password: email=" . $model->getEmail() . ", customer_id=" . $model->getId()
                . ", password=" . $password . ", current_hash_password=" . $model->getPasswordHash(), null, "debug_password.log");
    }
}
