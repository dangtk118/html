...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        $session = $this->_getSession();
        if(Fahasa_Customerregister_Helper_Data::isInternalFakeFHSEmail($customer->getEmail())){
            /**
             * By pass email confirmation if see email fahasatmdt.com,
             * as this is required by sale to handle phone order
             */
            ...
            $session->addSuccess($this->__('Account confirmation is required. Please, check your email (%s) for the confirmation link. To resend the confirmation email please <a href="%s">click here</a>.',
                    $customer->getEmail(),
        ...
        try {
            ...
            if ($customer->getConfirmation()) {
                ...
                try {
                    ...
                    // begin confirm telephone 
                    // break flow confirm email -> redirect to confirm telephone
                    $this->_redirectSuccess($this->_getUrl('tryout/telephone' , array('_secure' => true, "id"=>$id)));
                    return;
                    // end confirm telephone 
                    
                    // handle in telephone comfirm
//                    $customer->setConfirmation(null);
//                    $customer->save();
//                    //Custom event for account create to server future Fahasa campaign
