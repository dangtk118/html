...
/* end aggregator methods */
	...
	$is_get_all_validate_msg = Mage::getSingleton('customer/session')->getRuleMsg();
	$is_return_fail = false;
	
        foreach ($this->getConditions() as $cond) {
            $validated = $cond->validate($object, $all, $true);
	    
            if ($all && $validated !== $true) {
		if(empty($is_get_all_validate_msg)){
		    return false;
		}else{
		    $is_return_fail = true;
		}
