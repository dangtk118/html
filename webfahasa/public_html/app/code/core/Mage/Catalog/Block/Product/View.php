...
class Mage_Catalog_Block_Product_View extends Mage_Catalog_Block_Product_Abstract
{
    ...
    protected function _prepareLayout()
    {
		...
		$title = !empty($product['meta_title'])?$product['meta_title']:'';
		if ($title) {
		...
		if ($keyword) {
		    ...
		    $headBlock->setKeywords($product['name']);
		}
		$description = !empty($product['meta_description'])?$product['meta_description']:'';
		if ($description) {
		    $headBlock->setDescription( ($description) );
		} else {
		    $headBlock->setDescription(Mage::helper('core/string')->substr($product['description'], 0, 255));
		}
	    }else{
