...
class Mage_Api_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function wsiArrayUnpacker(&$obj)
    {
        if (is_object($obj)) {
            ...
            //Trung: properly handle array of complex type
            if (isset($obj->complexObjectArray)) {
                if (is_array($obj->complexObjectArray)) {
                    $obj = $obj->complexObjectArray;
                } else { // for one element array
                    $obj = array($obj->complexObjectArray);
                }
            }
