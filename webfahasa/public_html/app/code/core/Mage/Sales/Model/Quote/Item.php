...
class Mage_Sales_Model_Quote_Item extends Mage_Sales_Model_Quote_Item_Abstract
{
     ...
     * @return Mage_Sales_Model_Quote_Item
     */
    public function addQty($qty, $is_in_cart = false)
    {
        $oldQty = $this->getQty();
...
//        }
    ...
    public function getProductInCartNetcore($_qty = null){
	...
	$netcoreProductCotent['price_text'] = number_format(round($price, 2), 0, ",", ".");
        $netcoreProductCotent['prqt'] = ($_qty!=null?$_qty:(int)($this->_getData('qty'))*-1);
    ...
    public function getProductForEnhancedEcom($_qty = null)
    {
        $product = $this->getProduct();
        $final_price = doubleval($product->getFinalPrice());
        $qty = ($_qty!=null?$_qty:(int)($this->_getData('qty')));
        if ($qty < 0){
            $qty = $qty * (-1);
        }
        $name = addslashes($product->getName());
        $category_2 = addslashes($product->getCategoryMid());
        $result = "{'name': '{$name}', 'id': '{$product->getSku()}',"
        . "'category': '{$category_2}', "
        . "'brand': '{$product->getSupplier()}', 'price': '{$final_price}',"
                . "'quantity': {$qty}}";
        return $result;
    }
