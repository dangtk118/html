   ...
   /**
    ...
    public function addToCart(Mage_Checkout_Model_Cart $cart, $delete = false)
    {
        ...
        if (Mage::getStoreConfig('netcore/general/enable') == 1){
            ...
            $netcore .= "smartech('dispatch', 'Add To Cart',{\"items\": [".$this->getProductToCartNetcore()."]});";
            $netcore .= "smartech('dispatch', 'Remove from Wishlist',{\"items\": [".$this->getProductNetCore()."]});";
	    ...
	    $suggestion .= "Suggestion(SESSION_ID, 'Add To Cart',{\"items\": [".$this->getProductToCartNetcore()."]});";
        ...
        if (Mage::getStoreConfig('enhanced_ecom/general/enable') == 1){
            ...
            $enhanced_ecom .= "dataLayer.push({'event': 'addToCart', 'ecommerce': {'currencyCode': '{$currency}', 'add': {'products': ["
                    . $this->getProductForEnhancedEcom()
                    . "]}}});";
    ...
    public function getProductNetCore(){
        $product = $this->getProduct();
	...
	$netcoreProductCotent['final_price_text'] = number_format(round($final_price, 2), 0, ",", ".");
        ...
        $netcoreProductCotent['image'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA)."/catalog/product".$product->getSmallImage();
        ...
        return json_encode($netcoreProductCotent, JSON_UNESCAPED_UNICODE);
    }
    public function getProductToCartNetcore(){
        $product = $this->getProduct();
        if(Mage::helper('discountlabel')->getBundlePrice($product)){
            $price = $product->getData('price');
            $final_price = $product->getFinalPrice();
        ...
        $netcoreProductCotent['prid'] = (int)($product->getId());
        $netcoreProductCotent['name'] = $product->getName();
        $netcoreProductCotent['brand'] = $brand;
        $netcoreProductCotent['variant'] = '';
        $netcoreProductCotent['prqt'] = (int)$product->_getData('qty');
        $netcoreProductCotent['price'] = round($price, 2);
        $netcoreProductCotent['final_price'] = round($final_price, 2);
	$netcoreProductCotent['price_text'] = number_format(round($price, 2), 0, ",", ".");
	$netcoreProductCotent['final_price_text'] = number_format(round($final_price, 2), 0, ",", ".");
        $netcoreProductCotent['currency'] = $currency;
        $netcoreProductCotent['category_main'] = $product->getCategoryMain();
        $netcoreProductCotent['category_mid'] = $product->getCategoryMid();
        $netcoreProductCotent['image'] = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA)."/catalog/product".$product->getSmallImage();
        $netcoreProductCotent['url'] = Mage::getBaseUrl().$product->getUrlPath();
